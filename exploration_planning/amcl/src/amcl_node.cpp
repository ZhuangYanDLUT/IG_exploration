        ...
        first_reconfigure_call_(true)
{
  ...
  pose_pub_ = nh_.advertise<geometry_msgs::PoseWithCovarianceStamped>("amcl_pose", 2, true);      //发布pose
  particlecloud_pub_ = nh_.advertise<geometry_msgs::PoseArray>("particlecloud", 2, true);         //发布粒子
  global_loc_srv_ = nh_.advertiseService("global_localization", 
					 &AmclNode::globalLocalizationCallback,
                                         this);        //全局定位，利用均匀分布初始化粒子
  nomotion_update_srv_= nh_.advertiseService("request_nomotion_update", &AmclNode::nomotionUpdateCallback, this);    //无运动模型的更新？？？？
  set_map_srv_= nh_.advertiseService("set_map", &AmclNode::setMapCallback, this);       //设置新的地图
  ...
  laser_scan_filter_->registerCallback(boost::bind(&AmclNode::laserReceived,
                                                   this, _1));         //激光订阅
  initial_pose_sub_ = nh_.subscribe("initialpose", 2, &AmclNode::initialPoseReceived, this);//初始位姿

  if(use_map_topic_) {
    map_sub_ = nh_.subscribe("map", 1, &AmclNode::mapReceived, this);      //地图订阅
    ROS_INFO("amcl Subscribed to map topic.");
  } else {
    requestMap();
  }
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
  ...
  ROS_INFO("beam_skip_distance = %lf",beam_skip_distance_);
...
AmclNode::requestMap()
{
  ...
  ROS_INFO("amcl Requesting the map...");
