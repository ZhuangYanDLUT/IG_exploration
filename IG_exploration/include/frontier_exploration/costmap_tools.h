 ...
 * @param result Index of located cell最后得到的定位后的结果栅格
 * @param start Index initial cell to search from搜索的起始栅格
...
bool nearestCell(unsigned int &result, unsigned int start, unsigned char val, const costmap_2d::Costmap2D& costmap){
    ...
    while(!bfs.empty()){
        ...
        BOOST_FOREACH(unsigned nbr, nhood8(idx, costmap)){//将八邻域内的栅格坐标求出，并依次赋值给nbr
            if(!visited_flag[nbr]){
                bfs.push(nbr);
