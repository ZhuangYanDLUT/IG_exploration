...
public:
        ...
        retry_(5)
    {
        ...
        private_nh_.param<double>("goal_aliasing", goal_aliasing_, 0.1);   //误差
...
private:
     ...
     * 在接收到探索区域和探索点后运行executeCb函数
     */

    void executeCb(const frontier_exploration::ExploreTaskGoalConstPtr &goal)
    {

        success_ = false;    //是否得到探索点
        moving_ = false;      //是否发布探索点

        explore_costmap_ros_->resetLayers();     //初始化代价地图
        ...
        if(ros::ok() && as_.isActive()){
            ...
            if(updateBoundaryPolygon.call(srv)){    //updateBoundaryPolygon服务，发布探索区域
        ...
        //loop until all frontiers are explored查找探索点
        ...
        while(ros::ok() && as_.isActive()){
            ...
            //provide current robot pose to the frontier search service request边界探索服务需要的当前机器人的位资
            ...
            //check if robot is not within exploration boundary and needs to return to center of search area  忽略
            if(goal->explore_boundary.polygon.points.size() > 0 && !pointInPolygon(eval_pose.pose.position,goal->explore_boundary.polygon)){
                ...
                success_ = true;
                goal_pose = feedback_.next_frontier = srv.response.next_frontier;//服务，返回探索点
                retry_ = 5;
            ...
            if(!moving_ || !pointsNearby(move_client_goal_.target_pose.pose.position,goal_pose.pose.position,goal_aliasing_*0.5)){
                ROS_INFO("New exploration goal x=%lf,y=%lf,theta=%lf",goal_pose.pose.position.x,goal_pose.pose.position.y,goal_pose.pose.position.z);
                move_client_goal_.target_pose = goal_pose;
                boost::unique_lock<boost::mutex> lock(move_client_lock_);
                if(as_.isActive()){
                    move_client_.sendGoal(move_client_goal_, boost::bind(&FrontierExplorationServer::doneMovingCb, this, _1, _2),0,boost::bind(&FrontierExplorationServer::feedbackMovingCb, this, _1));  //发布探索目标点
                    moving_ = true;
                }
                lock.unlock();
            }
     ...
     */
    void preemptCb(){
            ROS_INFO("hhhhhhhhhhhhhhh");    //wlf add
        boost::unique_lock<boost::mutex> lock(move_client_lock_);
        move_client_.cancelGoalsAtAndBeforeTime(ros::Time::now());
        ROS_WARN("Current exploration task cancelled");

        if(as_.isActive()){
            ROS_INFO("llllllllllllllll");    //wlf add
            as_.setPreempted();
        }
