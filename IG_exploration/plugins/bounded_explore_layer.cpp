...
namespace frontier_exploration
{
    ...
    void BoundedExploreLayer::onInitialize(){
        ...
        marker_pub = nh_.advertise<visualization_msgs::MarkerArray>("marker_array1", 5);    //wlf add
        configured_ = false;
        marked_ = false;
    ...
    bool BoundedExploreLayer::getNextFrontier(geometry_msgs::PoseStamped start_pose, geometry_msgs::PoseStamped &next_frontier){
        ...
        //ROS_INFO("getNextFrontier service------------------------------------------------------");
        ...
        //get list of frontiers from search implementation  得到边界集合
        unsigned int index;   //wlf add
        visualization_msgs::MarkerArray markerArray;     //wlf add
        std::list<Frontier> frontier_list = frontierSearch.searchFrom(start_pose.pose.position,index,markerArray);
        ...
        Frontier selected;
        //selected.cost = std::numeric_limits<double>::infinity();
        selected.cost = -1;
        ...
        BOOST_FOREACH(Frontier frontier, frontier_list){
            //load frontier into visualization poitncloud  将边界链接点存入点云中
            ...
            for(int i=0;i<frontier.size;i++){   //wlf add
              frontier_point_viz.x = frontier.points[i].x;
              frontier_point_viz.y = frontier.points[i].y;
              frontier_cloud_viz.push_back(frontier_point_viz);
            }
        ...
        markerArray.markers[max].color.r = 100;
        //color selected frontier  将选择的边界链接点的颜色强度设置为100
        //frontier_cloud_viz[index].intensity = 100;
    ...
    bool BoundedExploreLayer::updateBoundaryPolygon(geometry_msgs::PolygonStamped polygon_stamped){
        ...
        polygon_.points.clear();//清理以存在的探索区域
        ...
        BOOST_FOREACH(geometry_msgs::Point32 point32, polygon_stamped.polygon.points){
            ...
            polygon_.points.push_back(costmap_2d::toPoint32(out.point));//将探索区域点压入polygon_
        }
